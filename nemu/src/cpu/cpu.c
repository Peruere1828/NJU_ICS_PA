...
void exec(uint32_t n)
{
	...
	while (n > 0 && nemu_state == NEMU_RUN)
	{
		...
		{
			instr_len = exec_inst();
		// 	if(cpu.eip >= 196669 && cpu.eip <= 196760){
		// 	printf("#Debug esp: ");
		// 	for(int i = 0;i < 5 && cpu.esp+4*i <= 0x07fffff0;i++){
		// 		tmp.addr = cpu.esp+4*i;
		// 		operand_read(&tmp);
		// 		printf("%u ",tmp.val);
		// 	}
		// 	printf("\n");
		// 	printf("#Debug ebp: ");
		// 	for(int i = 0;i < 5 && cpu.ebp+4*i <= 0x07fffff0;i++){
		// 		tmp.addr = cpu.ebp+4*i;
		// 		operand_read(&tmp);
		// 		printf("%u ",tmp.val);
		// 	}
		// 	printf("\n");
