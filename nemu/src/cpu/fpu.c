...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp,
  ...
  if (!overflow) {
    ...
    if (exp > 0 && (sig_grs >> (23 + 3)) == 1) {
      sig_grs = sig_grs & ((1 << (23 + 3)) - 1);
    }
    ...
    if (exp > 0 && (sig_grs >> 23) == 1) {
      sig_grs = sig_grs & ((1 << 23) - 1);
    }
  }
...
uint32_t internal_float_add(uint32_t b, uint32_t a) {
  ...
  uint32_t shift = (fb.exponent == 0 ? 1 : fb.exponent) -
                   (fa.exponent == 0 ? 1 : fa.exponent);
  assert(shift >= 0);
