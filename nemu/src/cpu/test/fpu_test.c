...
void fpu_test_add() {
  ...
  for (i = 0; i < sizeof(input) / sizeof(float); i++) {
    for (j = 0; j < sizeof(input) / sizeof(float); j++) {
      ...
      //   printf(
      //       "float add a = %f, b = %f, ua = %x, ub = %x, res = %x, res_fpu "
      //       "=%x,res = %f,res_fpu = %f\n ",
      //       a.fval, b.fval, a.val, b.val, res.val, res_fpu.val, res.fval,
      //       res_fpu.fval);
      fflush(stdout);
...
void fpu_test_sub() {
  ...
  for (i = 0; i < sizeof(input) / sizeof(float); i++) {
    for (j = 0; j < sizeof(input) / sizeof(float); j++) {
      ...
      // printf("float sub a = %f, b = %f, ua = %x, ub = %x, res = %x, res_fpu =
      // %x, res = %f, res_fpu = %f\n", a.fval, b.fval, a.val, b.val, res.val,
      // res_fpu.val, res.fval, res_fpu.fval);
      fflush(stdout);
      assert(res_fpu.val == res.val);
    }
  }
...
void fpu_test_div() {
  ...
  for (i = 0; i < sizeof(input) / sizeof(float); i++) {
    for (j = 0; j < sizeof(input) / sizeof(float); j++) {
      ...
      // printf("float div a = %f, b = %f, ua = %x, ub = %x, res = %x, res_fpu =
      // %x, res = %f, res_fpu = %f\n", a.fval, b.fval, a.val, b.val, res.val,
      // res_fpu.val, res.fval, res_fpu.fval); printf("a       sign %x, exp %x,
      // %d, sig %x\n", a.sign, a.exponent, a.exponent, a.significand);
      // printf("b       sign %x, exp %x, %d, sig %x\n", b.sign, b.exponent,
      // b.exponent, b.significand); printf("res     sign %x, exp %x, %d, sig
      // %x\n", res.sign, res.exponent, res.exponent, res.significand);
      // printf("res_fpu sign %x, exp %x, %d, sig %x\n", res_fpu.sign,
      // res_fpu.exponent, res_fpu.exponent, res_fpu.significand);
      fflush(stdout);
...
void fpu_test_mul() {
  ...
  for (i = 0; i < sizeof(input) / sizeof(float); i++) {
    for (j = 0; j < sizeof(input) / sizeof(float); j++) {
      ...
      //   printf(
      //       "float mul a = %f, b = %f, ua = %x, ub = %x, res = %x, res_fpu
      //       =%x, " "res = %f, res_fpu = %f\n", a.fval, b.fval, a.val, b.val,
      //       res.val, res_fpu.val, res.fval, res_fpu.fval);
      //   printf("a       sign %x, exp %x,%d, sig %x\n", a.sign, a.exponent,
      //          a.exponent, a.fraction);
      //   printf("b       sign %x, exp %x, %d, sig %x\n", b.sign, b.exponent,
      //          b.exponent, b.fraction);
      //   printf("res     sign %x, exp %x, %d, sig %x\n", res.sign,
      //   res.exponent,
      //          res.exponent, res.fraction);
      //   printf("res_fpu sign %x, exp %x, %d, sig %x\n", res_fpu.sign,
      //          res_fpu.exponent, res_fpu.exponent, res_fpu.fraction);
      fflush(stdout);
      assert(res_fpu.val == res.val);
    }
  }
